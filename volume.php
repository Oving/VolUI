...
<body>
	<div class="container">
		...
		<div class="panel panel-primary">
			<div class="panel-heading">
				<h3 class="panel-title">Create a new volume： <strong><span id="idtext"></span></strong>
			...
			<div  class="panel-body">
				...
				<table id="maintable" class="table table-condensed" >
					<tr class='nodrop nodrag'>
						<td width="15%"><h5>Volume Height：</h5></td>
						<td>
								<input id="textHeight" type="text">
						</td>
					...
					<tr class='nodrop nodrag'>
						<td colspan="2"><h5>Add a Vertex：<em> Like this: (X,Z) or X,Z       </em><a style="font-size: 120%; cursor:pointer;" onclick="addvertices();dragItems()"><span class="glyphicon glyphicon-plus" aria-hidden="true"></span></a></h5>
						</td> 
					</tr>
					<tr>
					...
					<tr class='nodrop nodrag'>
						<td colspan="4"><button onclick="saveVolume()" class="btn btn-primary">Save</button>
						&nbsp;&nbsp;
						<button onclick="cleanAll()" class="btn btn-primary">Clean</button>
						&nbsp;&nbsp;
						<button onclick="deleteVolume()" class="btn btn btn-danger">Delete this volume</button>
						</td>
					</tr>
				</table>
	...
	function drawCanvas()
	{
		...
		$('input[name="vertices"]').each(function(){
	        ...
	        vertices.push({ 'x': (+ArryVer[0]), 'z': (+ArryVer[1])});
		});

		if(vertices.length<3)
		{
			console.log('failed: Can not constitute a polygon.' );
			return;
		}
	...
	function showVolume() {        
            ...
            if(volume!=null)
            {
                $("#textHeight").val(volume["height"]);
                $("#textY").val(volume["y"]);
				volume["color"] = volume["color"].substr(-2) + volume["color"].slice(0,-2) ;
                reSetColor(volume["color"]);
                ...
                for (var i = 0; i < vertices.length; i++) {
                    ...
                    maintable.find('input:eq('+(3+i)+')').val('('+vertex["x"]+','+vertex["z"]+')');
                }
            }
            else
            	$('.btn-danger').remove();
        ...
        function deleteVolume(){
        	Nu4DataService.DeleteVolumeOfDevice(deviceId, function (deleted) {});
        	alert("Hava Deleted!");
        ...
        function saveVolume() {
        	var sign = 0;
            var height = parseFloat($("#textHeight").val());
            var y = parseFloat($("#textY").val());
            var color = $('#custom').spectrum("get").toHex8();
            color = color.slice(2) + color.slice(0,2);
            if (isNaN(deviceId) || isNaN(height) || isNaN(y) || color.length != 8) 
            	{
            		alert("Failed: Please check the necessary parameters again!");
            		return;
            	}
            var vertices = [];
            var tableVolume = $('#maintable');
            var vNum = tableVolume.find('input[name="vertices"]');
            vNum.each(function () {
                var InputVer = $(this).val();
				...
				if(isNaN(ArryVer[0]) || isNaN(ArryVer[1]) || ArryVer[0]==""|| ArryVer[1]=="" || ArryVer.length!=2)
                {
                	return false;//break
                }
                if(sign==1)
            ...
            if (vertices.length == 0 || (sign!=1 && (vertices.length!=vNum.length) )  || vertices.length<3) 
            	{
